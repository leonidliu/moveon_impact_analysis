...
drop table if exists election_2024.exp2024_basefile_20240917;
create table election_2024.exp2024_basefile_20240917 as (
    select *
    from base
    where
    ...
    select
        ...
        row_number() over (
            partition by
                ...
                vf_wireless_confidence asc, vf_phone_last_validated desc,
                vf_partisanship desc,
                ts_tsmart_harris_support_score desc,
                vf_biden_score desc,
                right(vb_voterbase_id, 2)
        ) as phone2
    from election_2024.exp2024_basefile_20240917
    where
        is_callable = 1  -- has high quality phone number in the voter file
        and do_not_call = 0 -- not on the do not call list
);

drop table if exists election_2024.exp2024_deduped_20240917;
create table election_2024.exp2024_deduped_20240917 as (
    select base.*
    from election_2024.exp2024_basefile_20240917 as base
    inner join zc_temp on base.vb_voterbase_id = zc_temp.vb_voterbase_id
    where vh_g2022 = 0 and phone2 = 1
);

---Check sept counts against aug: 
select
    case when prev.vb_voterbase_id is not NULL then 1 else 0 end as on_prev,
    case when cur.vb_voterbase_id is not NULL then 1 else 0 end as on_cur,
    count(*)
from election_2024.exp2024_ucg_august as prev
full join
    election_2024.exp2024_deduped_20240917  as cur
    on prev.vb_voterbase_id = cur.vb_voterbase_id
group by 1, 2;
