...
-- final assignment
drop table if exists election_2024.exp2024_ucg_sept;
create table election_2024.exp2024_ucg_sept as (
    select
        distinct
        coalesce(
            ind_april.treat,
    ...
    left join
        election_2024.exp2024_sept_vf_individual_assignment as ind
        on bf.vb_voterbase_id = ind.vb_voterbase_id
    left join 
        election_2024.exp2024_ucg_august as ind_aug
        on bf.vb_voterbase_id = ind_aug.vb_voterbase_id
    left join
        election_2024.exp2024_ucg_july as ind_july
        on bf.vb_voterbase_id = ind_july.vb_voterbase_id
    left join
    ...
    left join
        election_2024.exp2024_sept_update_householder_assignment as hh
        on bf.addr_id = hh.addr_id
    left join 
        hh_aug
        on bf.addr_id = hh_aug.addr_id
    left join
        election_2024.exp2024_ucg_july as hh_july
        on bf.addr_id = hh_july.addr_id
    left join
...
grant select on election_2024.exp2024_ucg_sept to redash_default;
...
from
    election_2024.exp2024_ucg_sept;
