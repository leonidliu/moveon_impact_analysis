...
drop table if exists election_2024.fl_basefile_20240821;
create table election_2024.fl_basefile_20240821 as (
    select *
    from base
    where
...
select distinct vf_best_phone from 
 election_2024.exp2024_ucg_aug_all
 );
...
(
    select
        fl.vb_voterbase_id,
        fl.vf_wireless_confidence,
        fl.vf_partisanship,
        fl.vb_vf_earliest_registration_date,
        fl.vf_best_phone,
        row_number() over (
            partition by
                fl.vf_best_phone order by
                fl.vf_wireless_confidence asc, vf_phone_last_validated desc,
                fl.vf_partisanship desc,
                fl.vf_biden_score desc,
                right(fl.vb_voterbase_id, 2)
        ) as phone2
    from election_2024.fl_basefile_20240821 fl
    left join aug using(vf_best_phone)
    where
        fl.is_callable = 1  -- has high quality phone number in the voter file
        and fl.do_not_call = 0 -- not on the do not call list
        and aug.vf_best_phone is null --- not in main list
);

drop table if exists election_2024.fl_deduped_20240821;
create table election_2024.fl_deduped_20240821 as (
    select distinct base.*
    from election_2024.fl_basefile_20240821 as base
    inner join zc_temp on base.vb_voterbase_id = zc_temp.vb_voterbase_id
    where vh_g2022 = 0 and phone2 = 1
);
grant select on election_2024.fl_deduped_20240821 to redash_default;

select count(*), count(distinct vb_voterbase_id) from  election_2024.fl_deduped_20240821;
