...
create temp table base as (
    select base.*
    from election_2024.exp2024_callable_expanded_oct as base
    left join election_2024.exp2024_ucg_unified_late_oct as prev
        on base.vb_voterbase_id = prev.vb_voterbase_id
    left join election_2024.exp2024_ucg_unified_late_oct as hh_prev
...
create temp table exp as (
    select
        ...
        case
            when base.vf_turnout_score between 0 and 20 then 'turnout_0to20'
                base.vf_turnout_score between 20 and 40
                then 'turnout_20to40' when
                base.vf_turnout_score between 40 and 60
                then 'turnout_40to60'
            when base.vf_turnout_score between 60 and 80 then 'vf_turnout_60to80'
            else 'turnout_80to100'
        end as categorical_turnout,
        case
            when base.vf_partisanship > 75 then '1' else '0'
        end as categorical_partisanship,
...
drop table if exists election_2024.exp2024_expanded_vf_individual_assignment;
create table election_2024.exp2024_expanded_vf_individual_assignment as (
    select
...
grant select
on election_2024.exp2024_expanded_vf_individual_assignment
to redash_default;
...
drop table if exists election_2024.exp2024_expanded_update_householder_assignment;
create table election_2024.exp2024_expanded_update_householder_assignment as (
    select
        hh_strata.addr_id,
        hh_strata.hh_size,
...
grant select
on election_2024.exp2024_expanded_update_householder_assignment
to redash_default;
