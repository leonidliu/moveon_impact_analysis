...
For the balance checks, I'm dropping the roughly 3K people that we were not able to join to the dataset.
...
clean_df <- filter(clean_df, !is.na(vb_tsmart_exact_track))
...
smd_summary_by_group <- function(data, group_var, treatment_var = treatment, 
  ...
  group_percents <- data %>%
    ...
    ungroup() %>%
    select(-n) %>%
    pivot_wider(names_from = {{ treatment_var }}, values_from = prop) %>%
    rename(Control = !!control_label, Treatment = !!treat_label)

  # Add group sizes by treatment (for n-weighted pooling)
  group_sizes <- data %>%
    count({{ treatment_var }}, {{ group_var }}, name = "n") %>%
    pivot_wider(names_from = {{ treatment_var }}, values_from = n) %>%
    rename(n_Control = !!control_label, n_Treatment = !!treat_label)
  
  # Final output
  group_percents %>%
    left_join(group_sizes, by = rlang::as_name(rlang::ensym(group_var))) %>%
    mutate(SMD = round((Treatment - Control) /
                         sqrt(((n_Treatment - 1) * Treatment * (1 - Treatment) + (n_Control  - 1) * Control  * (1 - Control)) /
                                (n_Treatment + n_Control - 2)), 3),
...
clean_df %>%
  ...
  mutate(Control = round(Control, 1),
...
# for 2022 election
placebo_glm_22 <- glm(formula = voted_22 ~ 
                     treat +
                     target_geography_state +
                     ts_tsmart_partisan_score +
                     ts_tsmart_partisan_score^2 +
...
# for 2020 election
placebo_glm_20 <- glm(formula = voted_20 ~ 
                     treat +
                     target_geography_state +
                     ts_tsmart_partisan_score +
                     ts_tsmart_partisan_score^2 +
