...
ai_estimates <- tribble(
      ...
      "Both",     both_n,         "Low",     0.1,
      "Both",     both_n,         "Medium",  0.3,
      "Both",     both_n,         "High",    1.0) %>%
...
#### Alternative way of calculating "impact scenarios"

Instead of using attempts, you can just use contacts with different effect sizes

```{r}
ai_estimates2 <- tibble(Scenario = c("Low", "Medium", "High"),
                       `Effect Size` = c(.005, .01, .05)) %>%
  mutate(`Net Votes` = sum(full_clean_df$exp_contacted) * `Effect Size`) %>%
  mutate(`Net Dem Votes` = `Net Votes` * adjustment_factor)
...
This was not in the initial analysis plan but it might be nice to have the results from all four analyses (OLS, 2SLS on the subset, 2SLS on the full dataset, and AI) in one table.

#### Pull OLS Results (subset)

```{r ols_effects_subset}
...
#### Pull OLS Results (full)

```{r ols_effects_full}
full_ols_effect_size <- regression_results %>% 
  filter(regression_type== "OLS full",
         condition == "treatment - control") %>%
  pull(predicted_prob)

full_ols_effect_size_low <- regression_results %>% 
  filter(regression_type== "OLS full",
         condition == "treatment - control") %>%
  pull(ci_low_90)

full_ols_effect_size_high <- regression_results %>% 
  filter(regression_type== "OLS full",
         condition == "treatment - control") %>%
  pull(ci_high_90)
...
#### Pull 2SLS Results (subset)

```{r tsls_subset_effects}
...
#### Pull 2SLS Results (full)

```{r tsls_full_effects}
full_twosls_effect_size <- regression_results %>% 
  filter(regression_type== "2SLS full",
         condition == "contacted - not contacted") %>%
  pull(predicted_prob)

full_twosls_effect_size_low <- regression_results %>% 
  filter(regression_type== "2SLS full",
         condition == "contacted - not contacted") %>%
  pull(ci_low_90)

full_twosls_effect_size_high <- regression_results %>% 
  filter(regression_type== "2SLS full",
         condition == "contacted - not contacted") %>%
  pull(ci_high_90)
...
#### Get Net Dem. Votes for All Four Analyses

```{r net_dem_votes}
#  Net Dem Votes
twosls_net_dem_votes <- num_contacted * twosls_effect_size * adjustment_factor
full_twosls_net_dem_votes <- num_contacted * full_twosls_effect_size * adjustment_factor
ols_net_dem_votes <- treatment_n * ols_effect_size * adjustment_factor
full_ols_net_dem_votes <- treatment_n * full_ols_effect_size * adjustment_factor
ai_net_dem_votes <- ai_estimates2 %>%
...
full_twosls_low_net_dem_votes <- num_contacted * full_twosls_effect_size_low * adjustment_factor
ols_low_net_dem_votes <- treatment_n * ols_effect_size_low * adjustment_factor
full_ols_low_net_dem_votes <- treatment_n * full_ols_effect_size_low * adjustment_factor
ai_low_net_dem_votes <- ai_estimates2 %>%
  filter(Scenario == "Low") %>%
  pull(`Net Dem Votes`)

# 2sls High 90 CI Net Dem Votes
twosls_high_net_dem_votes <- num_contacted * twosls_effect_size_high * adjustment_factor
full_twosls_high_net_dem_votes <- num_contacted * full_twosls_effect_size_high * adjustment_factor
ols_high_net_dem_votes <- treatment_n * ols_effect_size_high * adjustment_factor
full_ols_high_net_dem_votes <- treatment_n * full_ols_effect_size_high * adjustment_factor
ai_high_net_dem_votes <- ai_estimates2 %>%
...
```{r print_table}
tibble(
  Analysis = c("OLS (subset)", "OLS (full)", "2SLS (subset)", "2SLS (full)", "Meta-Analysis"),
  Effect = round(c(ols_effect_size, full_ols_effect_size, twosls_effect_size, full_twosls_effect_size, NA), 3),
  `Low Effect` = round(c(ols_effect_size_low, full_ols_effect_size_low, twosls_effect_size_low, full_twosls_effect_size_low, NA), 3),
  `High Effect` = round(c(ols_effect_size_high, full_ols_effect_size_high, twosls_effect_size_high, full_twosls_effect_size_high, NA), 3),
  `Net Dem Votes` = round(c(ols_net_dem_votes, full_ols_net_dem_votes, twosls_net_dem_votes, full_twosls_net_dem_votes, ai_net_dem_votes)),
  `Low Net Dem Votes` = round(c(ols_low_net_dem_votes, full_ols_low_net_dem_votes, twosls_low_net_dem_votes, full_twosls_low_net_dem_votes, ai_low_net_dem_votes)),
  `High Net Dem Votes` = round(c(ols_high_net_dem_votes, full_ols_high_net_dem_votes, twosls_high_net_dem_votes, full_twosls_high_net_dem_votes, ai_high_net_dem_votes))) %>%
  kable(format.args = list(big.mark = ","))
```
