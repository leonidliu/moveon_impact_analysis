...
postcard_n <- full_clean_df %>%
  filter(postcard_contacted == 1,
         any_phone_contacted == 0) %>%
  nrow()

phone_n <- full_clean_df %>%
  filter(postcard_contacted == 0,
         any_phone_contacted == 1) %>%
  nrow()
...
ai_estimates <- tribble(
      ~Mode,      ~`N Contacted`, ~Scenario, ~`Tot. Effect`,
  ...
  mutate(`Net Votes`  = `Tot. Effect` * `N Contacted` / 100,
